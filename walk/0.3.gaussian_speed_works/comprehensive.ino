...
void orient_randomly() {
  int new_orientation = random(0, 12);
...
void track() {
  ...
  double val = gaussian.random();
	double currentSpeed = val;
  // units in um/min. Ensure not faster than maximum possible neutrophil spd.
  currentSpeed = min(25.0, currentSpeed);	
  // cell's can't move backwards
	currentSpeed = abs(currentSpeed);
  currentSpeed = currentSpeed/25.0*200; //200 is maxspeed of robot
  orient_randomly();
  run_speed(currentSpeed);
  delay(500);
