...
{
//  int i;
//  for (i = 0; i < 3; i++) //Output one frequency sound
//  {
...
void IR_Deal()
{
  if (irrecv.decode(&results)) //Infrared received result
  {
    switch (results.value)
    {
      ...
      case 0x00FFC23D: g_AllState = 1; g_modeSelect = 2;  ModeBEEP(g_modeSelect); break; // NEXT obstacle avoidance mode  2
      case 0x00FFE01F: g_AllState = 1; g_modeSelect = 4;  ModeBEEP(g_modeSelect); break; // PREVIOUS obstacle avoidance mode  2
      case 0x00FF906F: g_AllState = 1; g_modeSelect = 1;  ModeBEEP(g_modeSelect); break; // Back line walking mode  3
...
void spin_right_360() {
  spin_right();
  delay(890); //Right rotation for 300ms
  brake(); //stop
  delay(1000);
}
...
void spin_left_360() {
  spin_left();
  delay(950); //Right rotation for 300ms
  brake(); //stop
  delay(1000);
}
...
void neutrophil_inverse_crw() {
  ...
  pitch *= (1.0 - correlationFactor) + (correlationFactor * 
           (scaledMaxSpeed - scaledSpeed) / scaledMaxSpeed);
  // 360/12 = 30 degree turn resolution
  double new_orientation = orientation+pitch/(M_PI*2)*12;
  orient(new_orientation);
  // scale to 230 PWM, which is the maxSpeed of robot
  run_speed(currentSpeed/maxSpeed*230);
  // walk at selected speed for 500 ms
  delay(500);
}

void neutrophil_brownian_walk() {
  double speedStd = 10;
  gaussian.mean = 0;
  gaussian.variance = speedStd*speedStd;
  double currentSpeed = gaussian.random();
...
void loop()
{
  ...
  if (g_AllState == 0)
  {
    switch (g_modeSelect)
    {
      ...
      case 4: neutrophil_brownian(); break;// Tracking mode
    }
  }
  CarControl();
}
